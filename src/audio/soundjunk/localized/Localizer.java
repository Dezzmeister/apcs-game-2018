...
public class Localizer {
	...
	/**
	 * The gain at maxDistance. Not guaranteed to be the minimum gain, otherwise
	 * sounds may always be heard regardless of distance.
	 */
	...
	public float findBalance(List<Speaker> speakers) {
		...
		for (Speaker s : speakers) {
			if (s.isOn()) {
				...
				float angleCos = RenderUtils.cosOfAngleBetweenLines(new Wall(listenerDirection, listener),
						new Wall(listener, speakerLocation));
	...
	/**
	 * Uses a formula for finding the total resistance in a parallel circuit to
	 * determine total gain based on distance.
	 *
	 * @param speakers
	 * @return
	 */
