...
public class Camera {
	...
	private void computeSideDir() {
		float x = dir.x * COS_T - dir.y * SIN_T;
		float y = dir.x * SIN_T + dir.y * COS_T;
		
		sidedir = new Vector2(x, y);
	}
	
	/**
	 * Used with LUT rotation methods. This method takes a potential rotation factor
	 * and computes sine, cosine, and negative sine. It puts these values in a
	 * lookup table to avoid computing them in real time. The lookup table returns
	 * the sine of the key: to get cosine, use (HALF_PI+key); to get negative sine,
	 * use (TWO_PI-speed).
	 *
	 * @param factor
	 */
	...
	/**
	 * A faster implementation of <code>rotateLeft()</code> that uses a lookup
	 * table. Use in conjunction with <code>preComputeFactor()</code>.
	 *
	 * @param factor
	 *            multiplied by internal rotation speed and used to rotate
	 */
	...
	/**
	 * A faster implementation of <code>rotateRight()</code> that uses a lookup
	 * table. Use in conjunction with <code>preComputeFactor()</code>.
	 *
	 * @param factor
	 *            multiplied by internal rotation speed and used to multiply
	 */
	 ...
	 *
	 * @param factor
	 *            multiplied by the rotation speed, product used to rotate left
	 */
	 ...
	 *
	 * @param factor
	 *            multiplied by the rotation speed, product used to rotate right
	 */
