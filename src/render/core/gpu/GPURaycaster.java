...
public class GPURaycaster extends Raycaster {
	...
	private static final long serialVersionUID = -267034786990360375L;
	private GPURenderer renderer = new GPURenderer();
	...
	public GPURaycaster(Game _parentGame, Camera _camera, WorldMap _worldMap, int resWidth, int resHeight,
			int renderWidth, int renderHeight, int threads) {
		super(_parentGame, _camera, _worldMap, resWidth, resHeight, renderWidth, renderHeight, threads);
	}
			...
			dda: while (!hit) {
				...
				if (block != Block.SPACE) {
					if (block.isCustom()) {
						...
						float currentLocx = mapX;
						float currentLocy = mapY;
						float rayDirectionx = (float) (rdirx + posx);
						float rayDirectiony = (float) (rdiry + posy);
