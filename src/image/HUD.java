	...
	public HUD autoFindBars() {
		...
		for (int y = 0; y < HEIGHT; y++) {
			for (int x = 0; x < WIDTH; x++) {
				...
				} else if (x > 0 && pixels[x + y * WIDTH] != HEALTH_COLOR && pixels[(x - 1) + y * WIDTH] == HEALTH_COLOR
						&& y < HEIGHT - 1 && pixels[(x - 1) + ((y + 1) * WIDTH)] != HEALTH_COLOR && health2 == null) {
					health2 = new Point(x, y);
				} else if (x > 0 && pixels[x + y * WIDTH] != COFFEE_COLOR && pixels[(x - 1) + y * WIDTH] == COFFEE_COLOR
						&& y < HEIGHT - 1 && pixels[(x - 1) + ((y + 1) * WIDTH)] != COFFEE_COLOR && coffee2 == null) {
