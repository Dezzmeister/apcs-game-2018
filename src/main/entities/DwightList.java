...
public class DwightList {
	...
	public void updateDwights(double delta) {
		...
		for (int i = dwights.size() - 1; i >= 0; i--) {
			...
			if (dwight.health.get() <= 0) {
				...
				if (world.getFloorAt((int) removed.pos.x,
						(int) removed.pos.y) == Block.DwightElements.STANDARD_HALL_FLOOR) {
	...
	private boolean spawnRandomDwight() {
		 ...
		 * for (int x = px - radius; x <= px; x++) { for (int y = py - radius; y <= py;
		 * y++) { int clause = ((x - px) * (x - px)) + ((y - py) * (y - py));
		 * 
		 * if (clause <= radius * radius && clause >= (radius - 1) * (radius - 1)) { int
		 * xSym = px - (x - px); int ySym = py - (y - py); //(x,y) (x,ySym) (xSym, y)
		 * (xSym, ySym) are in circle
		 * 
		 * } } } }
		 */
