...
public class Dwight extends Actor {
	...
	private Queue<Node> determinePathTo(Vector2 player) {
		...
		Queue<Node> open = new PriorityQueue<Node>(
				(a, b) -> Float.compare(fMap.getOrDefault(a, 0f), fMap.getOrDefault(b, 0f)));
	...
	private void identifySuccessors(Node node, Node goal, Set<Node> goals, Queue<Node> open, Set<Node> closed,
			Map<Node, Node> parentMap, Map<Node, Float> fMap, Map<Node, Float> gMap, Map<Node, Float> hMap) {
